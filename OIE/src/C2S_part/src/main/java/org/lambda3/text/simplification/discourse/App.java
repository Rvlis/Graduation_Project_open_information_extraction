...
public class App {
    ...
    public static void main(String[] args) throws IOException {
        ...
        SimplificationContent content = DISCOURSE_SIMPLIFIER.doDiscourseSimplification(new File("../data/C2S_input.txt"), ProcessingType.SEPARATE, true);
        content.serializeToJSON(new File("../data/output.json"));
        // saveLines(new File("output_default.txt"), Arrays.asList(content.defaultFormat(false)));
        saveLines(new File("../data/C2S_output.txt"), Arrays.asList(content.flatFormat(false)));
        // LOGGER.info("done");
    }
}
